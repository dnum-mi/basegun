...
const selectedOption = computed(() => {
  if (typology.securingSteps[0].text) {
    return typology.securingSteps[
      store.selectedOptions.length - 1
    ].options.find((option) => option.value === store.selectedOptions.at(-1));
  }
...
<template>
  <div class="fr-container">
    <div class="fr-col-12 fr-col-lg-6 mx-auto">
      ...
      <h2 class="text-center my-auto text-blue">Manipulations</h2>
      <div>
        <div class="fr-col-sm-6 fr-col-lg-12 mx-auto">
          <div class="fr-content-media relative">
            ...
            <span class="absolute -bottom-1.5rem right-0 text-sm"
              >Environ {{ selectedOption.timeVideo }}</span
            >
          </div>
        </div>
